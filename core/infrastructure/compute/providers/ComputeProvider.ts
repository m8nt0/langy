import { LocalComputerAdapter, CloudComputeAdapter, EdgeComputeAdapter, P2PComputeAdapter } from "../adapters";
...
export class ComputerAdapter {
    ...
    private initializeAdapters() {
        ...
        if (this.config.local) {
            this.adapters.set('local', new LocalComputerAdapter(this.config.local.maxWorkers));
        }
        if (this.config.cloud) {
            this.adapters.set('cloud', new CloudComputeAdapter(this.config.cloud));
        }
        if (this.config.edge) {
            this.adapters.set('edge', new LocalComputerAdapter(this.config.edge.nodes));
        }
        if (this.config.p2p) {
